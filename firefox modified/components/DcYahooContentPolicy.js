/*
  An XPcom component that stops Yahoo from tracking the webpages you go to.
 ...
 * @constructor
 */
function DcYahooContentPolicy() {
	//allows access of native methods by scripts in content area
	this.wrappedJSObject = this;  	
}

DcYahooContentPolicy.prototype =
{
	classDescription: "This is an image blocker",
	contractID: "@disconnect.me/dcyahoo/contentpolicy;1",
	classID: Components.ID("{d32a3c00-4ed3-11de-8a39-0800200c9a76}"),
		...
		if(reqOrig != null && reqOrig.host!="browser" && contLoc.host!="browser" && contLoc.host!="global" && contType!=6){
			...
			if( reqOrig.host !=contLoc.host && !this.isMatching(reqOrig.host, this.DOMAINS) && this.isMatching(contLoc.host, this.DOMAINS) && typeof aContext.ownerDocument != null){
				...
				try{
					...
					if(typeof aContext.ownerDocument.DcYahooCount == "undefined"){
						aContext.ownerDocument.DcYahooCount = 1;
					}
					else{
						aContext.ownerDocument.DcYahooCount += 1;						
					}					
					
					var finalBlocking = aContext.ownerDocument.defaultView.content.localStorage.getItem('DcYahooStatus');
...
if (XPCOMUtils.generateNSGetFactory)
	var NSGetFactory = XPCOMUtils.generateNSGetFactory([DcYahooContentPolicy]);
else
	var NSGetModule = XPCOMUtils.generateNSGetModule([DcYahooContentPolicy]);
