 ...
 * @constructor
 */
function DcFbdcContentPolicy() {
	//allows access of native methods by scripts in content area
	this.wrappedJSObject = this;  	
}

DcFbdcContentPolicy.prototype =
{
	classDescription: "This is an image blocker",
	contractID: "@disconnect.me/dcfbdc/contentpolicy;1",
	classID: Components.ID("{d32a3c00-4ed3-11de-8a39-0800200c9a71}"),
		...
		if(reqOrig != null && reqOrig.host!="browser" && contLoc.host!="browser" && contLoc.host!="global"){
			...
			if( reqOrig.host !=contLoc.host && !this.isMatching(reqOrig.host, this.DOMAINS) && this.isMatching(contLoc.host, this.DOMAINS) && typeof aContext.ownerDocument != null){
				...
				try{
					...
					var finalBlocking = aContext.ownerDocument.defaultView.content.localStorage.getItem('DcFbdcStatus');
...
if (XPCOMUtils.generateNSGetFactory)
	var NSGetFactory = XPCOMUtils.generateNSGetFactory([DcFbdcContentPolicy]);
else
	var NSGetModule = XPCOMUtils.generateNSGetModule([DcFbdcContentPolicy]);
